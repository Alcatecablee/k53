...
async function migrateScenarios() {
  ...
  try {
    ...
    for (let i = 0; i < k53ScenarioBank.length; i += batchSize) {
      ...
      if (insertError) {
        console.error(`Error inserting scenarios batch ${i / batchSize + 1}:`, insertError);
        return;
      }

      console.log(`Inserted scenarios batch ${i / batchSize + 1}/${Math.ceil(k53ScenarioBank.length / batchSize)}`);
    }
...
async function migrateQuestions() {
  ...
  try {
    ...
    for (let i = 0; i < k53QuestionBank.length; i += batchSize) {
      ...
      console.log(`Inserted questions batch ${i / batchSize + 1}/${Math.ceil(k53QuestionBank.length / batchSize)}`);
    }
