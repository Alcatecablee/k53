...
-- This script creates all necessary tables for the achievement system
...
CREATE TABLE IF NOT EXISTS achievements (
    ...
    category VARCHAR(50) NOT NULL CHECK (category IN ('scenarios', 'questions', 'location', 'streak', 'mastery')),
...
-- User achievements table (tracks user progress and unlocks)
...
CREATE TABLE IF NOT EXISTS achievement_notifications (
    ...
    title VARCHAR(255) NOT NULL,
    description TEXT NOT NULL,
    type VARCHAR(20) NOT NULL DEFAULT 'unlocked' CHECK (type IN ('unlocked', 'shared', 'milestone')),
...
CREATE TABLE IF NOT EXISTS achievement_history (
    ...
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    achievement_id UUID NOT NULL REFERENCES achievements(id) ON DELETE CASCADE,
    action VARCHAR(20) NOT NULL CHECK (action IN ('unlocked', 'shared', 'viewed', 'exported', 'imported')),
    metadata JSONB,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS achievement_analytics (
    ...
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    achievement_id UUID NOT NULL REFERENCES achievements(id) ON DELETE CASCADE,
    time_to_unlock INTEGER, -- in minutes
    attempts_before_unlock INTEGER DEFAULT 0,
    category_performance JSONB, -- stores performance by category
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Achievement exports table
CREATE TABLE IF NOT EXISTS achievement_exports (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    export_data JSONB NOT NULL,
    export_format VARCHAR(20) NOT NULL DEFAULT 'json' CHECK (export_format IN ('json', 'csv', 'pdf')),
    file_name VARCHAR(255),
    file_size INTEGER,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Achievement leaderboards table
...
CREATE TABLE IF NOT EXISTS achievement_shares (
    ...
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    achievement_id UUID NOT NULL REFERENCES achievements(id) ON DELETE CASCADE,
    share_platform VARCHAR(50), -- 'twitter', 'facebook', 'whatsapp', etc.
    share_url TEXT,
...
CREATE INDEX IF NOT EXISTS idx_achievement_history_created_at ON achievement_history(created_at);
CREATE INDEX IF NOT EXISTS idx_achievement_analytics_user_id ON achievement_analytics(user_id);
...
ALTER TABLE achievement_exports ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "achievements_admin_insert" ON achievements FOR INSERT WITH CHECK (auth.role() = 'admin');
CREATE POLICY "achievements_admin_update" ON achievements FOR UPDATE USING (auth.role() = 'admin');
CREATE POLICY "achievements_admin_delete" ON achievements FOR DELETE USING (auth.role() = 'admin');

-- RLS Policies for user_achievements (user can only access their own data)
CREATE POLICY "user_achievements_user_access" ON user_achievements FOR ALL USING (auth.uid() = user_id);

-- RLS Policies for achievement_notifications (user can only access their own notifications)
CREATE POLICY "achievement_notifications_user_access" ON achievement_notifications FOR ALL USING (auth.uid() = user_id);

-- RLS Policies for achievement_history (user can only access their own history)
CREATE POLICY "achievement_history_user_access" ON achievement_history FOR ALL USING (auth.uid() = user_id);

-- RLS Policies for achievement_analytics (user can only access their own analytics)
CREATE POLICY "achievement_analytics_user_access" ON achievement_analytics FOR ALL USING (auth.uid() = user_id);

-- RLS Policies for achievement_exports (user can only access their own exports)
CREATE POLICY "achievement_exports_user_access" ON achievement_exports FOR ALL USING (auth.uid() = user_id);

-- RLS Policies for achievement_leaderboards (public read for leaderboards, user write for own data)
...
-- RLS Policies for achievement_shares (user can only access their own shares)
CREATE POLICY "achievement_shares_user_access" ON achievement_shares FOR ALL USING (auth.uid() = user_id);
