...
export const getDatabaseLogs: RequestHandler = async (req, res) => {
  try {
    ...
    const logs = systemLogs.filter(log => 
      log.type === 'database' || log.type === 'system'
    ).slice(0, 50);
...
export const backupDatabase: RequestHandler = async (req, res) => {
  try {
    ...
    logEvent('system', `Database backup created with ${Object.keys(backup.tables).length} tables`);
...
export const toggleMaintenanceMode: RequestHandler = async (req, res) => {
  try {
    ...
    logEvent('system', `Maintenance mode ${maintenanceMode ? 'enabled' : 'disabled'}`);
