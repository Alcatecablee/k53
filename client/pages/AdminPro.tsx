...
export default function AdminPro() {
  ...
  const toggleMaintenanceMode = async () => {
    try {
      const response = await safeFetch("/api/enterprise/maintenance-mode", {
        method: "POST",
      ...
      if (response.ok) {
        ...
        throw new Error(`Toggle maintenance mode failed: ${response.statusText}`);
      }
    } catch (error) {
  ...
  const triggerBackup = async () => {
    try {
      const response = await safeFetch("/api/enterprise/backup", {
        method: "POST",
      });
      ...
      if (response.ok) {
        const result = await response.json();
        if (result.success) {
          setLastBackupTime(new Date().toLocaleDateString());
          alert("Backup triggered successfully!");
        } else {
          alert("Failed to trigger backup.");
        }
      } else {
        throw new Error(`Trigger backup failed: ${response.statusText}`);
      }
    } catch (error) {
  ...
  const runSecurityScan = async () => {
    try {
      const response = await safeFetch("/api/enterprise/security-scan", {
        method: "POST",
      });
      ...
      if (response.ok) {
        const result = await response.json();
        if (result.success) {
          setLastSecurityScan(new Date().toLocaleDateString());
          alert("Security scan triggered successfully!");
        } else {
          alert("Failed to trigger security scan.");
        }
      } else {
        throw new Error(`Run security scan failed: ${response.statusText}`);
      }
    } catch (error) {
      console.error("Run security scan error:", error);
      alert(`Failed to run security scan: ${error}`);
    }
  };

  const saveConfiguration = async () => {
    try {
      // Get form values using proper type casting
      const sessionTimeoutInput = document.querySelector('input[type="number"]') as HTMLInputElement;
      const failedLoginInput = document.querySelectorAll('input[type="number"]')[1] as HTMLInputElement;
      const ipWhitelistTextarea = document.querySelector('textarea') as HTMLTextAreaElement;
      const currencySelect = document.querySelector('select') as HTMLSelectElement;
      const taxRateInput = document.querySelectorAll('input[type="number"]')[2] as HTMLInputElement;
      const smtpServerInput = document.querySelector('input[type="text"]') as HTMLInputElement;
      const fromEmailInput = document.querySelector('input[type="email"]') as HTMLInputElement;

      const response = await safeFetch("/api/enterprise/configuration", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          maintenanceMode: maintenanceMode,
          sessionTimeout: sessionTimeoutInput?.value || 30,
          failedLoginAttempts: failedLoginInput?.value || 5,
          twoFactorAuth: false,
          ipWhitelist: ipWhitelistTextarea?.value || "",
          currency: currencySelect?.value || "ZAR",
          taxRate: taxRateInput?.value || 15,
          smtpServer: smtpServerInput?.value || "",
      ...
      if (response.ok) {
        ...
        throw new Error(`Save configuration failed: ${response.statusText}`);
      }
    } catch (error) {
