...
const DEFAULT_SEO: SEOData = {
  ...
  keywords: 'K53, learners licence, driving test, South Africa, Department of Transport, practice test, driving exam, road signs, traffic rules',
  ogType: 'website',
...
export function useSEO(seoData: Partial<SEOData> = {}) {
  ...
  useEffect(() => {
    ...
    updateMetaTag('robots', `${data.noIndex ? 'noindex' : 'index'},${data.noFollow ? 'nofollow' : 'follow'}`);
    ...
    if (data.ogImage) {
      updateMetaTag('og:image', data.ogImage.startsWith('http') ? data.ogImage : baseUrl + data.ogImage, 'property');
    ...
    if (data.ogImage) {
      updateMetaTag('twitter:image', data.ogImage.startsWith('http') ? data.ogImage : baseUrl + data.ogImage);
      updateMetaTag('twitter:image:alt', data.ogImageAlt || data.title);
    }
    
    // Additional SEO meta tags
    updateMetaTag('author', 'Department of Transport, Republic of South Africa');
