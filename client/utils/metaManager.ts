...
class MetaManager {
  ...
  public setPageSEO(seo: {
    ...
    if (seo.ogImage) {
      ...
      this.updateMeta({ property: 'og:image:alt', content: seo.ogImageAlt || seo.title });
      this.updateMeta({ property: 'og:image:width', content: '1200' });
      this.updateMeta({ property: 'og:image:height', content: '630' });
    }

    // Twitter Card tags
    this.updateMeta({ name: 'twitter:card', content: seo.twitterCard || 'summary_large_image' });
    ...
    if (seo.ogImage) {
      ...
      this.updateMeta({ name: 'twitter:image:alt', content: seo.ogImageAlt || seo.title });
    }

    // Additional SEO meta tags
    this.updateMeta({ name: 'author', content: 'Department of Transport, South Africa' });
    this.updateMeta({ name: 'publisher', content: 'SuperK53' });
    this.updateMeta({ name: 'application-name', content: 'SuperK53' });
    this.updateMeta({ name: 'apple-mobile-web-app-title', content: 'SuperK53' });
  ...
  public setPerformanceOptimizations(): void {
    ...
    this.updateLink({ rel: 'preconnect', href: 'https://fonts.googleapis.com' });
    this.updateLink({ rel: 'preconnect', href: 'https://fonts.gstatic.com', crossorigin: 'anonymous' });
  ...
  public setMobileOptimizations(): void {
    ...
    this.updateMeta({ name: 'apple-mobile-web-app-status-bar-style', content: 'default' });
    this.updateMeta({ name: 'apple-mobile-web-app-title', content: 'SuperK53' });
    ...
    // Touch icons
    this.updateLink({ rel: 'apple-touch-icon', href: '/apple-touch-icon.png', sizes: '180x180' });
    this.updateLink({ rel: 'icon', href: '/favicon-32x32.png', sizes: '32x32', type: 'image/png' });
    this.updateLink({ rel: 'icon', href: '/favicon-16x16.png', sizes: '16x16', type: 'image/png' });
...
export function updatePageSEO(seoData: Parameters<typeof metaManager.setPageSEO>[0]) {
