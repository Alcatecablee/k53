...
import { X, Download, Edit, Save, Cancel, Copy, RefreshCw } from 'lucide-react';
        ...
        <Tabs defaultValue="overview" className="w-full">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <p className="text-sm text-blue-700">
                    Since {user?.subscription?.created_at ? new Date(user.subscription.created_at).toLocaleDateString() : 'Unknown'}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600">Daily Scenarios</span>
                    <span className="text-sm font-medium">
                      {user?.usage?.scenarios_used || 0} / {user?.usage?.max_scenarios === -1 ? '∞' : (user?.usage?.max_scenarios || 5)}
                    </span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      ...
                      style={{ 
                        width: user?.usage?.max_scenarios === -1 ? '100%' : 
          ...
          <TabsContent value="usage" className="space-y-6 mt-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                <ProgressRing 
                  progress={
                    user?.usage?.max_scenarios === -1 ? 100 : 
                    Math.min(100, ((user?.usage?.scenarios_used || 0) / (user?.usage?.max_scenarios || 5)) * 100)
                  }
                  size={120}
                  color="#3B82F6"
          ...
          <TabsContent value="activity" className="space-y-6 mt-6">
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {[
                  { action: 'Completed scenario', time: '2 hours ago', type: 'success' },
                  { action: 'Started practice session', time: '4 hours ago', type: 'info' },
                  { action: 'Updated profile', time: '1 day ago', type: 'info' },
                  { action: 'Subscribed to Pro plan', time: '3 days ago', type: 'success' },
                ].map((activity, index) => (
...
export const SystemLogModal: React.FC<SystemLogModalProps> = ({
  ...
  const getLogTitle = () => {
    switch (logType) {
      case 'database': return 'Database Logs';
      case 'error': return 'Error Logs';
      case 'access': return 'Access Logs';
      case 'security': return 'Security Threats';
      default: return 'System Logs';
    }
  };
